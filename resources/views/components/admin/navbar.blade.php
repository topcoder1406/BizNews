<nav aria-label="secondary" x-data="{ open: false }"
    ...
    <div class="flex items-center gap-3">
        <x-admin.button type="button" class="md:hidden" iconOnly variant="secondary" srText="Toggle dark mode"
            @click="toggleTheme">
    ...
    <div class="flex items-center gap-3">
        <x-admin.button type="button" class="hidden md:inline-flex" iconOnly variant="secondary" srText="Toggle dark mode"
            @click="toggleTheme">
            <x-heroicon-o-moon x-show="!isDarkMode" aria-hidden="true" class="w-6 h-6" />
            <x-heroicon-o-sun x-show="isDarkMode" aria-hidden="true" class="w-6 h-6" />
        </x-admin.button>
        <x-admin.dropdown align="right" width="48">
            ...
            <x-slot name="content">
                ...
                <form method="POST" action="{{ route('admin.logout') }}">
                    ...
                    <x-admin.dropdown-link :href="route('admin.logout')" onclick="event.preventDefault();
...
<div class="fixed inset-x-0 bottom-0 flex items-center justify-between px-4 py-4 sm:px-6 transition-transform duration-500 bg-white md:hidden dark:bg-dark-eval-1"
    ...
    <div>
        <x-admin.application-logo aria-hidden="true" class="w-10 h-10" />
        <span class="sr-only">K UI</span>
    </div>

    <x-admin.button type="button" iconOnly variant="secondary" srText="Open main menu"
